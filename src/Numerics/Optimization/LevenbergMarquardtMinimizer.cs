...
{
    public sealed class LevenbergMarquardtMinimizer : IUnconstrainedMinimizer
    {
        ...
        public static MinimizationResult Minimum(IObjectiveFunction objective, Vector<double> initialGuess, double initialMu = 1E-3, double gradientTolerance = 1E-18, double stepTolerance = 1E-18, double functionTolerance = 1E-18, int maximumIterations = -1)
        {
            ...
            if (!objective.IsGradientSupported)
                throw new IncompatibleObjectiveException("Gradient is not supported in objective function, but required for Levenberg-Marquardt minimization.");
            if (!objective.IsHessianSupported)
                throw new IncompatibleObjectiveException("Hessian is not supported in objective function, but required for Levenberg-Marquardt minimization.");
            ...
            return new MinimizationResult(objective, iterations, exitCondition);
        }
    }
}
